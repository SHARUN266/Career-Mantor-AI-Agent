...
export const AiCareerChatAgent = createAgent({
  ...
  description: "An Ai Agent that answer career related question",
...
export const AiResumeAnalyzerAgent=createAgent({
  ...
  description:'AI Resume Analyzer Agent help to Resturn Report',
...
export const AiResumeAgent = inngest.createFunction(
  ...
  { event: "AiResumeAgent" },
  async ({ event, step }) => {
    const { recordId, base64ResumeFile, pdfToText } = await event.data;
    // Upload pdf to Cloud

    const uploadImageKitURL = await step.run("uploadImage", async () => {
    ...
    const aiResumeReport=await AiResumeAnalyzerAgent.run(pdfToText);
     //@ts-ignore
